...
bool TextClass::Initialize(ID3D10Device* device, HWND hwnd, int screenWidth, int screenHeight, D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../Directx10KinectFramework/data/fontdata.txt", L"../Directx10KinectFramework/data/font.dds");
	if(!result)
	{
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, "Pos: " , 20, 60, 0.0f, 1.0f, 0.0f);
	if(!result)
	{
		return false;
	}
...
bool TextClass::SetPos(float x, float y, float z)
{
	char xString[16];
	char yString[16];
	char zString[16];
	...
	// Setup the cpu string.
	strcpy_s(posString, "Pos: X: ");
	strcat_s(posString, xString);
	strcat_s(posString, " Y: ");
	strcat_s(posString, yString);
	strcat_s(posString, " Z: ");
	strcat_s(posString, zString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, posString, 20, 60, red, green, blue);
	if(!result)
	{
		return false;
	}
